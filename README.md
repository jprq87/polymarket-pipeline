...
- [Dashboard](#dashboard)
- [Data Analysis and Visualization with Bruin's AI Analyst](#data-analysis-and-visualization-with-bruins-ai-analyst)
...
- **Data Analysis (AI Analyst):** Generates complex secondary aggregations and analytical models automatically via natural language prompting, accelerating the path from core fact tables to BI-ready insights without manual SQL authoring.
...
graph LR
    ...
    classDef ai fill:#f0abfc,stroke:#333,stroke-width:1px,color:#000;
    ...
    J(reports.fct_daily_expiry_bucket_liquidity):::ai
    K(reports.fct_daily_category_concentration):::ai
...
**`reports.fct_daily_expiry_bucket_liquidity`** — Aggregates daily liquidity metrics into time-to-expiry buckets (`0-1d`, `2-7d`, `8-30d`, `31d+`, `expired`). Excludes markets without an `end_date` and provides an `expiry_bucket_order` numeric sort key for correct dashboard ordering. Partitioned by `date`, clustered by `expiry_bucket`. *(Generated by Bruin's AI Analyst — see [AI Analyst section](#data-analysis-and-visualization-with-bruins-ai-analyst))*

**`reports.fct_daily_category_concentration`** — Calculates per-market tick share and concentration metrics within each daily category slice. Includes category-level summary metrics (`top_market_share`, `top_3_market_share`, `hhi_concentration_index`) repeated on every row within the `(date, category)` partition for BI consumption without secondary joins. Partitioned by `date`, clustered by `category, market_id`. *(Generated by Bruin's AI Analyst — see [AI Analyst section](#data-analysis-and-visualization-with-bruins-ai-analyst))*
...
## Data Analysis and Visualization with Bruin's AI Analyst

To accelerate the delivery of actionable BI metrics and explore new paradigms in data modeling, this pipeline leverages Bruin's native AI Data Analyst to improve the analytical layer.

Rather than manually writing the complex window functions, statistical formulas, and conditional logic required for advanced secondary reporting, the AI Analyst was used to intuitively interrogate the core fact tables. By providing natural language prompts regarding the desired business outcomes, the tool automatically generated the underlying SQL for two distinct, high-value analytical assets:

- **`reports.fct_daily_expiry_bucket_liquidity`**: We wanted to understand how market liquidity behaves as an event approaches its resolution date. The AI Analyst generated the precise conditional logic to group markets into standardized time-to-expiry buckets (e.g., `0-1d`, `2-7d`, `8-30d`). This reveals whether trading activity heavily concentrates right before a market expires or remains steady throughout its lifecycle.
- **`reports.fct_daily_category_concentration`**: We needed to determine if the high tick volume within a category (like Politics or Crypto) was distributed evenly across dozens of markets or monopolized by a single viral event. The AI Analyst successfully translated this requirement into the mathematical SQL logic for the Herfindahl-Hirschman Index (HHI), alongside top-market share metrics, providing a mathematically rigorous measure of category dominance.

---

### Using Bruin Cloud's AI Features

<details>
<summary>Prerequisites — connecting your pipeline to Bruin Cloud</summary>

While this pipeline runs locally or on a standalone VM via cron, it can also be imported into **Bruin Cloud** for fully managed orchestration and AI-driven analysis. To get started:

**1. Create an Account** — Sign up at [cloud.getbruin.com](https://cloud.getbruin.com). This gives you access to a serverless orchestration environment with built-in scheduling, alerting, and SOC 2-compliant data governance.

**2. Link the GitHub Repository** — Bruin Cloud is Git-native. In the dashboard, connect your GitHub account and select the `polymarket-pipeline` repository. Bruin will automatically scan the repo, parse your `.bruin.yml` and `pipeline.yml` files, and visually map the full dependency graph without any extra configuration.

**3. Create the Necessary Connections** — Navigate to **Settings → Connections** and create a new GCP connection matching the name `bruin_gcp`. Paste the contents of your `polymarket-sa.json` service account key into the secure credential vault. Bruin Cloud injects these at runtime, the same way Secret Manager does on the Compute VM.

</details>

Once your pipeline has run and the report tables are populated in BigQuery, navigate to the **AI** tab in Bruin Cloud to explore your data without writing SQL.

**AI Chats (Data Analyst):** Use the conversational interface to ask complex ad-hoc questions about your pipeline data. The agent reads your documented asset schemas, column descriptions, and dependency graph to generate grounded, context-aware SQL — and returns plain-text analytical explanations alongside the results. For example: *"Why was there a spike in spread volatility in the Politics category on March 14th?"*

**AI Dashboarding:** Generate full dashboards from a natural language prompt. The agent uses your pipeline's existing asset structure and grain definitions to write the queries and render the charts automatically. You can inspect the underlying SQL for any chart, verify its accuracy, and iterate via follow-up prompts.

> The AI features work best when your asset headers are well-documented — grain definitions, column descriptions, and `depends` chains feed directly into the context the agent uses. The documentation work done on this pipeline is what made the generated assets coherent and structurally correct on the first pass.

---

### 📸 Visual Walkthrough: Bruin Cloud AI Features

The following screenshots demonstrate the pipeline's integration with Bruin Cloud, specifically showcasing how the managed AI features were used to architect the final analytical layer and visualize the data.

#### AI Data Analyst (Chats)

The conversational AI feature was used as a data engineering co-pilot. By grounding the agent in the project's repository documentation and SQL assets, it was able to propose, validate, and write the code for new reporting metrics.

**1. Schema Extraction & Context Awareness**
![AI Schema Analysis](images/bruin-chat-1.png)
*Prompting the agent to read `polymarket-pipeline/README.md` and the staging assets to understand the raw data grain. The AI successfully mapped the full lineage from `stg_orderbook` to the core fact tables.*

**2. Ideation & Metric Proposals**
![AI Dashboard Opportunities](images/bruin-chat-2.png)
*Acting as a data analyst, the agent reviewed the schema and proposed high-value derived metrics, specifically focusing on actionable visualization ideas.*

**3. Cost-Controlled Live Validation**
![AI Live Validation](images/bruin-chat-3.png)
*Instructing the agent to validate its proposed metrics against live BigQuery data. The AI ran highly constrained queries (`LIMIT` / 1-day partitions) to verify data distributions, proving that time-to-expiry significantly impacts spread and confirming category monopolization.*

**4. Automated Asset Generation**
![AI Asset Generation](images/bruin-chat-4.png)
![AI Asset Generation](images/bruin-chat-5.png)
*After successfully validating the logic, the agent automatically authored the production-ready SQL for `fct_daily_expiry_bucket_liquidity` and `fct_daily_category_concentration`, seamlessly transitioning from exploratory analysis to persistent data assets.*

#### AI Dashboarding

Bruin's AI Dashboard Builder was used to rapidly prototype visualizations. The agent iterated on the dashboard layout entirely through natural language commands, querying the fact tables directly.

**1. Architecting the Initial Dashboard**
![AI Dashboard Generation](images/bruin-dashboard-1.png)
*Asking the AI to architect an initial dashboard layout based solely on its understanding of the pipeline's documentation and data model, ensuring all queries are heavily optimized.*

**2. Iterative Refinement & Pruning**
![AI Dashboard Refinement](images/bruin-dashboard-2.png)
*Refining the dashboard via chat. Unwanted metrics were removed, and the agent was instructed to source the time-to-expiry and category concentration charts directly from the existing fact tables.*

**3. Proxy Resolution & Final Charting**
![AI Dashboard Proxy and Final](images/bruin-dashboard-3.png)
*The agent correctly mapped the requested "traded amount" to the available `tick_count` metric. After verifying this proxy with the user, it generated the final pie and 100% stacked bar charts to visualize category dominance.*
...
│   │   │   ├── fct_daily_category_concentration.asset.sql    # Bruin's AI Analyst suggestion
│   │   │   └── fct_daily_expiry_bucket_liquidity.asset.sql   # Bruin's AI Analyst suggestion
│   │   └── staging/
...
│   ├── pipeline.yml                                          # Bruin pipeline config (schedule, start_date, default connection)
│   └── README.md
├── terraform/
