...
The staging layer splits into two parallel tracks: `fetch_markets` builds the market dimension, while `stg_price_changes` processes the orderbook event stream. These streams are joined in the reports layer to build the foundational fact tables. The final two assets (`fct_daily_expiry_bucket_liquidity` and `fct_daily_category_concentration`) act as secondary aggregations, building exclusively on top of the core report tables rather than querying staging directly.
...
Daily snapshot per individual market: YES token price action (`yes_avg_bid`, `yes_avg_ask`, `yes_min_bid`, `yes_max_ask`), spread volatility (`STDDEV`), and a Pearson correlation of YES bid price against `UNIX_SECONDS(timestamp_received)` as a sentiment proxy (`+1` = strong YES trend, `-1` = strong NO trend). Partitioned by `date`, clustered by `category, market_id`.

---

### `reports.fct_daily_expiry_bucket_liquidity` ✨ Created by Bruin's AI Analyst

**Type:** BigQuery SQL · **Depends on:** `reports.fct_daily_market_history`

> Generated by Bruin's AI Analyst feature and reviewed/refined manually.

Aggregates daily liquidity metrics into time-to-expiry buckets. One row per `(date, expiry_bucket)` — maximum 5 rows per date.

| Bucket | Condition |
|---|---|
| `0-1d` | 0–1 days to expiry |
| `2-7d` | 2–7 days to expiry |
| `8-30d` | 8–30 days to expiry |
| `31d+` | more than 30 days to expiry |
| `expired` | past expiry date |

- Markets without an `end_date` are excluded — they cannot be placed on the expiry curve
- `expiry_bucket_order` provides a numeric sort key (1–5) for correct dashboard ordering
- Partitioned by `date`, clustered by `expiry_bucket`

---

### `reports.fct_daily_category_concentration` ✨ Created by Bruin's AI Analyst

**Type:** BigQuery SQL · **Depends on:** `reports.fct_daily_market_history`, `reports.fct_daily_category_activity`

> Generated by Bruin's AI Analyst feature and reviewed/refined manually.

Calculates per-market tick share and concentration metrics within each daily category slice. One row per `(date, category, market_id)`.

- `market_tick_share`: this market's fraction of total category ticks that day
- `top_market_share`, `top_3_market_share`, `hhi_concentration_index`: category-level summary metrics repeated on every row within the `(date, category)` partition for BI consumption without secondary joins
- HHI approaches 1 for monopoly (one market dominates), approaches 0 for evenly distributed activity
- Partitioned by `date`, clustered by `category, market_id`
...
- `fct_daily_expiry_bucket_liquidity`: bucket labels and sort keys match expected values
- `fct_daily_category_concentration`: market shares sum to 1.0 within each category-day
...
- `fct_daily_expiry_bucket_liquidity`: no more than 5 distinct buckets per day
- `fct_daily_category_concentration`: HHI index remains within valid range `[0, 1]`
